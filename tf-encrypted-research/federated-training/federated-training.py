"""Example of the secure aggregation protocol for federating learning."""
import argparse
import sys
...
from tf_encrypted.keras.datasets import *
...
class ModelOwner:
    ...
    Args:
        ...
        data_name: `str`, name of the dataset that model trained on
    """
    ...
    def _build_model(self):
        with tf.device(self.player.device_name):
            ...
            if (self.test_dataset.num_classes > 1):
                self.loss = tf.keras.losses.CategoricalCrosssentropy(from_logits=True)
                self.metric = lambda y_true, y_pred: tf.reduce_mean(
    ...
    def _build_update_func(self):
        ...
        def update_func(*grads):
            ...
            with tf.name_scope("update model"):
    ...
    def validate_model(self):
        with tf.device(self.player.device_name):
            ...
            # There's a potential bug in tensorflow when using `model.fit` or `model.evaluate`
            # within tf.device context the program will stop
...
class DataOwner:
    ...
    Args:
        ...
        model_name: `str`, name of the model to be trained.
        data_name: `str`, name of the dataset that the model was trained on
    ...
    def __init__(self, player_name, model_name, data_name, data_slice):
        self.player = tfe.get_config().get_player(name_or_player=player_name)
        self.model_name = model_name
        self.data_name = data_name
    ...
    def update_model(self, *update_weights):
        """Update model with the new weights from model owner"""
        with tf.device(self.player.device_name):
            weights = self.model.weights
...
def federated_training(model_owner, data_owners):
    # Share the model owner's model weights to data owners
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        help="Name of the dataset which model trained on",
    )
